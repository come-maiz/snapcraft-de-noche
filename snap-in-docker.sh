...
docker run -v "$(pwd)":/cwd ubuntu:xenial sh -c "apt update && apt install locales -y && locale-gen en_US.UTF-8 && export LC_ALL=en_US.UTF-8 && cd /cwd && $TRAVIS_BUILD_DIR/snap.sh $repo $repo_branch $ppa"
